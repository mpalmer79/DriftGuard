...
class SensorModel:
    ...
    def read(self, state: VehicleState, active_faults: Iterable[FaultRecord]) -> SensorReading:
        ...
        for fault in active_faults:
            ...
            ftype = fault.type
            step_offset = state.step - fault.start_step
            meta = resolve_metadata(fault.metadata, step_offset)
            if ftype == FaultType.SENSOR_DRIFT:
                magnitude = float(meta.get("magnitude", 1.5))
                self._drift_acc += magnitude
                fields = meta.get("affected_fields") or ["altitude"]
                ...
                magnitude = float(meta.get("magnitude", 50.0))
                altitude += magnitude
                ...
                amp = float(meta.get("magnitude", 15.0))
                ...
                p = float(meta.get("probability", 1.0))
